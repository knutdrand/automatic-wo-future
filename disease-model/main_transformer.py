...
This model uses TransformerModel with historical weather as past_covariates.
TransformerModel is a "block" model that predicts output_chunk_length steps at once,
...
class DiseaseModelConfig(BaseConfig):
    ...
    input_chunk_length: int = 12  # Look-back window for Transformer
    output_chunk_length: int = 3  # Direct 3-step prediction
    d_model: int = 32  # Transformer model dimension
...
async def on_train(
    ...
    """Train TransformerModel per location with historical weather covariates."""
    df = data.to_pandas()
    ...
    for location in locations:
        ...
        try:
            # TransformerModel - supports past_covariates
...
async def on_predict(
    ...
    """Generate predictions using trained Transformer models."""
    models = model["models"]
    ...
    for location in locations:
        ...
        if location not in models:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                loc_model = TransformerModel.load(model_path)
...
# Service metadata
info = MLServiceInfo(
    display_name="Transformer Disease Model (Historical Weather)",
    version="1.0.0",
    summary="Spatio-temporal disease prediction using Transformer with historical weather",
    description="Uses TransformerModel with past_covariates for historical climate data (rainfall, temperature) and Fourier seasonal features. Future predictions use seasonal features only.",
    author="CHAP Team",
