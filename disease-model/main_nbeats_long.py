"""N-BEATS model with LONGER input window.

Hyperparameter changes from baseline:
- input_chunk_length: 12 -> 24 (2 years lookback)
"""
...
class DiseaseModelConfig(BaseConfig):
    """Configuration for LONG input N-BEATS model."""

    input_chunk_length: int = 24  # Increased from 12 (2 years)
...
info = MLServiceInfo(
    display_name="NBEATS Disease Model (LONG INPUT)",
    version="1.0.0",
    summary="N-BEATS with 24-month lookback window (2 years)",
    description="N-BEATS model with longer input window to capture multi-year patterns.",
    author="CHAP Team",
