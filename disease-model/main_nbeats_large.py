"""N-BEATS model with LARGER architecture.
...
class DiseaseModelConfig(BaseConfig):
    """Configuration for LARGE N-BEATS model."""

    input_chunk_length: int = 12
    output_chunk_length: int = 3
    num_stacks: int = 15  # Increased from 10
    num_blocks: int = 1
    num_layers: int = 4  # Increased from 2
    layer_widths: int = 128  # Increased from 64
    n_epochs: int = 50  # Increased from 30
    batch_size: int = 32
    n_samples: int = 100
    min_dispersion: float = 1.0
    dispersion_scale: float = 0.1
...
info = MLServiceInfo(
    display_name="NBEATS Disease Model (LARGE)",
    version="1.0.0",
    summary="N-BEATS with larger architecture (128-wide, 4 layers, 15 stacks)",
    description="Larger N-BEATS model with more capacity for complex patterns.",
    author="CHAP Team",
